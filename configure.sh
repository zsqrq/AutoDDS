...
if test "x$TOOLSET" = x; then
  guessed_toolset=`CXX= CXXFLAGS= $my_dir/scripts/build.sh --guess-toolset`
  echo "Now using tool set is : ... $guessed_toolset "
...
if test "x$BJAM" = x; then
  $ECHO "Building B2 engine.. $TOOLSET"
  echo ${TOOLSET}
  pwd=`pwd`
  CXX= CXXFLAGS= "$my_dir/scripts/build.sh" ${TOOLSET}
  if [ $? -ne 0 ]; then
      echo
