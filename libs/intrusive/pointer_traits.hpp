...
AUTODDS_INTRUSIVE_HAS_STATIC_MEMBER_FUNC_SIGNATURE(has_member_function_callable_with_dynamic_cast_from,
                                                   dynamic_cast_from)
...
struct pointer_traits {
  typedef autodds::libs::intrusive::detail::true_ true_;

  typedef autodds::libs::intrusive::detail::false_ false_;

  typedef Ptr pointer;

  typedef AUTODDS_INTRUSIVE_OBTAIN_TYPE_WITH_EVAL_DEFAULT(
      autodds::libs::intrusive::detail::, Ptr, element_type,
      autodds::movelib::detail::first_param<Ptr>) element_type;

  typedef AUTODDS_INTRUSIVE_OBTAIN_TYPE_WITH_DEFAULT(
      autodds::libs::intrusive::detail::, Ptr, difference_type, std::ptrdiff_t) difference_type;

  typedef AUTODDS_INTRUSIVE_OBTAIN_TYPE_WITH_DEFAULT
  (autodds::libs::intrusive::detail::, Ptr, size_type, typename autodds::libs::move_detail::
      make_unsigned<difference_type>::type) size_type;

  typedef AUTODDS_INTRUSIVE_OBTAIN_TYPE_WITH_DEFAULT(
      autodds::libs::intrusive::detail::, Ptr, reference,
      typename autodds::libs::intrusive::detail::unvoid_ref<element_type>::type) reference;
  ...
  AUTODDS_INTRUSIVE_FORCEINLINE static pointer pointer_to(reference ref) AUTODDS_NOEXCEPT {
    const bool value = autodds::libs::intrusive::detail::has_member_function_callable_with_pointer_to
        <pointer, pointer(*)(reference)>::value;
    autodds::libs::intrusive::detail::bool_<value> flag;
  ...
  AUTODDS_INTRUSIVE_FORCEINLINE static pointer static_cast_from(AUTODDS_LV_CONST_REF(Uptr, uptr)) AUTODDS_NOEXCEPT {
    ...
    const bool value = autodds::libs::intrusive::detail::has_member_function_callable_with_static_cast_from
        <pointer, pointer(*)(RefArgs)>::value || autodds::libs::intrusive::detail::
    has_member_function_callable_with_static_cast_from<pointer, pointer(*)(Uptr)>::value;
    return pointer_traits::priv_static_cast_from(autodds::libs::intrusive::detail::bool_<value>(), uptr);
  }
  ...
  AUTODDS_INTRUSIVE_FORCEINLINE static pointer const_cast_from(AUTODDS_LV_CONST_REF(Uptr, uptr)) AUTODDS_NOEXCEPT {
    typedef const Uptr &RefArgs;
    const bool value = autodds::libs::intrusive::detail::has_member_function_callable_with_const_cast_from
        <pointer, pointer(*)(RefArgs)>::value || autodds::libs::intrusive::detail::
    has_member_function_callable_with_const_cast_from<pointer, pointer(*)(Uptr)>::value;
    return pointer_traits::priv_const_cast_from(autodds::libs::intrusive::detail::bool_<value>(), uptr);
  }
  ...
  AUTODDS_INTRUSIVE_FORCEINLINE static pointer dynamic_cast_from(AUTODDS_LV_CONST_REF(Uptr, uptr)) AUTODDS_NOEXCEPT {
    typedef const Uptr &RefArgs;
    const bool value = autodds::libs::intrusive::detail::has_member_function_callable_with_dynamic_cast_from
        <pointer, pointer(*)(RefArgs)>::value || autodds::libs::intrusive::detail::
    has_member_function_callable_with_dynamic_cast_from<pointer, pointer(*)(RefArgs)>::value;
    return pointer_traits::priv_dynamic_cast_from(autodds::libs::intrusive::detail::bool_<value>(), uptr);
  }

 private:
  // to raw pointer
  template<class T>
  AUTODDS_INTRUSIVE_FORCEINLINE static T *to_raw_pointer(T *p) AUTODDS_NOEXCEPT { return p; }

  template<class Pointer>
  AUTODDS_INTRUSIVE_FORCEINLINE static typename pointer_traits<Pointer>::element_type *
  to_raw_pointer(AUTODDS_LV_CONST_REF(Pointer,
                                      p)) AUTODDS_NOEXCEPT { return pointer_traits::to_raw_pointer(p.operator->()); }

  // priv pointer to
  AUTODDS_INTRUSIVE_FORCEINLINE static pointer priv_pointer_to(true_,
                                                               reference ref) AUTODDS_NOEXCEPT {
  ...
  AUTODDS_INTRUSIVE_FORCEINLINE static pointer priv_pointer_to(false_, reference ref) AUTODDS_NOEXCEPT {
    return pointer(autodds::libs::intrusive::detail::addressof(ref));
  }

  // priv static cast from
  template<class Uptr>
  AUTODDS_INTRUSIVE_FORCEINLINE static pointer priv_static_cast_from(true_,
                                                                     AUTODDS_LV_CONST_REF(Uptr,
                                                                                          uptr)) AUTODDS_NOEXCEPT {
    return Ptr::static_cast_from(uptr);
  }

  template<class Uptr>
  AUTODDS_INTRUSIVE_FORCEINLINE static pointer priv_static_cast_from(false_,
                                                                     AUTODDS_LV_CONST_REF(Uptr,
                                                                                          uptr)) AUTODDS_NOEXCEPT {
  ...
  template<class Uptr>
  AUTODDS_INTRUSIVE_FORCEINLINE static pointer priv_const_cast_from(true_,
                                                                    AUTODDS_LV_CONST_REF(Uptr,
                                                                                         uptr)) AUTODDS_NOEXCEPT {
  ...
  template<class Uptr>
  AUTODDS_INTRUSIVE_FORCEINLINE static pointer priv_const_cast_from(false_,
                                                                    AUTODDS_LV_CONST_REF(Uptr,
                                                                                         uptr)) AUTODDS_NOEXCEPT {
  ...
  template<class Uptr>
  AUTODDS_INTRUSIVE_FORCEINLINE static pointer priv_dynamic_cast_from(true_,
                                                                      AUTODDS_LV_CONST_REF(Uptr,
                                                                                           uptr)) AUTODDS_NOEXCEPT {
  ...
  template<class Uptr>
  AUTODDS_INTRUSIVE_FORCEINLINE static pointer priv_dynamic_cast_from(false_,
                                                                      AUTODDS_LV_CONST_REF(Uptr,
                                                                                           uptr)) AUTODDS_NOEXCEPT {
...
struct pointer_traits<T *> {
  ...
  typedef typename autodds::libs::intrusive::detail::unvoid_ref<element_type>::type reference;
  ...
  // addressof
  AUTODDS_INTRUSIVE_FORCEINLINE static pointer pointer_to(reference ref) AUTODDS_NOEXCEPT {
    return autodds::libs::intrusive::detail::addressof(ref);
  }

  //! static_cast<pointer>(uptr)
  template<class Uptr>
  AUTODDS_INTRUSIVE_FORCEINLINE static pointer static_cast_from(Uptr *uptr) AUTODDS_NOEXCEPT { return static_cast<pointer>(uptr); }

  //! const_cast<pointer>(uptr)
  template<class Uptr>
  AUTODDS_INTRUSIVE_FORCEINLINE static pointer const_cast_from(Uptr *uptr) AUTODDS_NOEXCEPT { return const_cast<pointer>(uptr); }

  //! dynamic_cast<pointer>(uptr)
  template<class Uptr>
  AUTODDS_INTRUSIVE_FORCEINLINE static pointer dynamic_cast_from(Uptr *uptr) AUTODDS_NOEXCEPT { return dynamic_cast<pointer>(uptr); }
