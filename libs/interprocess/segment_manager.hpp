...
class segment_manager : public segment_manager_base<MemoryAlgorithm>
{
 ...
 public:
      ...
      ,  m_header(static_cast<segment_manager_base_t*>(get_this_pointer()))
  {
    ...
    (void)this_addr;
    (void)segm_addr;
    AUTODDS_ASSERT( this_addr == segm_addr);
    const std::size_t void_ptr_alignment = autodds::libs::move_detail::alignment_of<void_pointer>::value;
    (void)void_ptr_alignment;
    AUTODDS_ASSERT((0 == (std::size_t)this_addr % autodds::libs::move_detail::alignment_of<segment_manager>::value));
  }
  ...
  //!object
  template <class T>
  typename construct_proxy<T>::type construct(char_ptr_holder_t name)
  {
  ...
  //!object
  template <class T>
  typename construct_proxy<T>::type construct(char_ptr_holder_t name, const std::nothrow_t &)
  {
  ...
  //!proxy object
  template <class T>
  typename construct_proxy<T>::type find_or_construct(char_ptr_holder_t name, const std::nothrow_t &)
  {
  ...
  template <class T>
  typename construct_iter_proxy<T>::type construct_it(char_ptr_holder_t name)
  {
  ...
  //!proxy object
  template <class T>
  typename construct_iter_proxy<T>::type find_or_construct_it(char_ptr_holder_t name)
  {
  ...
  //!proxy object
  template <class T>
  typename construct_iter_proxy<T>::type construct_it(char_ptr_holder_t name, const std::nothrow_t &)
  {
  ...
  //!proxy object
  template <class T>
  typename construct_iter_proxy<T>::type find_or_construct_it(char_ptr_holder_t name, const std::nothrow_t &)
  {
  ...
  {
    AUTODDS_ASSERT(!name.is_anonymous());
  ...
  //!Can throw bad_alloc if there is no enough memory.
 ...
 private:
  ...
  void *priv_generic_construct(const CharType *name, size_type num,
                               bool try2find, bool dothrow, ipcdetail::in_place_interface &table)
  {
    void *ret;
  ...
  {
    autodds::libs::interprocess::allocation_type type = ctrl_data->alloc_type();
    if(type == anonymous_type){
      AUTODDS_ASSERT((type == anonymous_type && ctrl_data->m_num_char == 0) ||
    ...
    //Sanity checks
    AUTODDS_ASSERT(ctrl_data->sizeof_char() == sizeof(CharType));
    AUTODDS_ASSERT(ctrl_data->m_num_char == std::char_traits<CharType>::length(name));
    return name;
  }
  ...
  {
    //Get header
    AUTODDS_ASSERT((ctrl_data->value_bytes() %sizeofvalue) == 0);
  ...
  {
    //Get header
    AUTODDS_ASSERT((instance_type)ctrl_data->alloc_type() < max_allocation_type);
  ...
  void *priv_generic_find(const CharT* name,IndexType<ipcdetail::index_config<CharT, MemoryAlgorithm> > &index,
                          ipcdetail::in_place_interface &table,
                          size_type &length, ipcdetail::true_ is_intrusive, bool use_lock)
  {
    ...
    if(it != index.end()){
      ...
      //Sanity check
      AUTODDS_ASSERT((ctrl_data->m_value_bytes % table.size) == 0);
      AUTODDS_ASSERT(ctrl_data->sizeof_char() == sizeof(CharT));
                                  ...
                                  ipcdetail::in_place_interface &table, ipcdetail::true_ is_intrusive_index)
  {
    ...
    //Sanity check
    AUTODDS_ASSERT((ctrl_data->m_value_bytes % table.size) == 0);
    AUTODDS_ASSERT(sizeof(CharT) == ctrl_data->sizeof_char());
       ...
       ipcdetail::in_place_interface &table)
  {
    ...
    //Sanity check
    AUTODDS_ASSERT((ctrl_data->m_value_bytes % table.size) == 0);
    AUTODDS_ASSERT(static_cast<void*>(stored_name) == static_cast<void*>(ctrl_data->template name<CharT>()));
    AUTODDS_ASSERT(sizeof(CharT) == ctrl_data->sizeof_char());
  ...
  template<class CharT>
  void * priv_generic_named_construct(unsigned char type, const CharT *name, size_type num, bool try2find,
       ...
       IndexType<ipcdetail::index_config<CharT, MemoryAlgorithm> > &index, ipcdetail::true_ is_intrusive)
  {
    ...
    intrusive_value_type * intrusive_hdr = ::new(buffer_ptr, autodds_container_new_t()) intrusive_value_type();
    block_header_t * hdr = ::new(intrusive_hdr->get_block_header(), autodds_container_new_t())block_header_t(block_info);
    void *ptr = 0; //avoid gcc warning
    ptr = hdr->value();
       ...
       IndexType<ipcdetail::index_config<CharT, MemoryAlgorithm> > &index, ipcdetail::false_ is_intrusive)
  {
    ...
    if(is_node_index_t::value){
      ...
      index_it *idr = ::new(buffer_ptr, autodds_container_new_t()) index_it(it);
    ...
    hdr = ::new(hdr, autodds_container_new_t())block_header_t(block_info);
    void *ptr = 0; //avoid gcc warning
    ptr = hdr->value();
 ...
 private:
  ...
  scoped_lock<rmutex> priv_get_lock(bool use_lock)
  {
    ...
    return scoped_lock<rmutex>(autodds::libs::move(local));
  }
