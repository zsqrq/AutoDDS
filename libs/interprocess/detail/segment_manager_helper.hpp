...
#ifndef AUTODDS_LIBS_INTERPROCESS_DETAIL_SEGMENT_MANAGER_HELPER_HPP_
#define AUTODDS_LIBS_INTERPROCESS_DETAIL_SEGMENT_MANAGER_HELPER_HPP_
...
template <typename MemoryManeger>
...
struct block_header
{
  ...
  size_type value_bytes() const { return m_value_bytes; }

  template<class Header>
  size_type total_size_with_header() const
  {
    return get_rounded_size(size_type(sizeof(Header)),
                            size_type(autodds::libs::container::dtl::alignment_of<block_header<size_type>>::value))
        + total_size();
  }
  ...
  {
    return block_header_from_value(value, sizeof(T), autodds::libs::container::dtl::alignment_of<T>::value);
  }

  static block_header<size_type>* block_header_from_value(const void* value, std::size_t sz, std::size_t algn)
  {
    block_header* hdr = const_cast<block_header*>(
        move_detail::force_ptr<const block_header*>(reinterpret_cast<const char *>(value) -
            get_rounded_size(sizeof(block_header), algn)));
    (void)sz;
    AUTODDS_ASSERT(hdr->m_value_alignment == algn);
    AUTODDS_ASSERT(hdr->m_value_bytes % sz == 0);
    return hdr;
  }
...
template <typename Hook, typename CharType, typename SizeType>
struct intrusive_value_type_impl : public Hook
{
 private:
  //Non-copyable
 ...
 public:
  ...
  enum
  {
    BlockHdrAlignment = ::autodds::libs::container::dtl::alignment_of<block_header<size_type>>::value
  };
...
template <typename CharT, typename MemoryAlgorithm>
struct index_config
{
...
template <typename Iterator, bool intrusive>
...
#endif //AUTODDS_LIBS_INTERPROCESS_DETAIL_SEGMENT_MANAGER_HELPER_HPP_