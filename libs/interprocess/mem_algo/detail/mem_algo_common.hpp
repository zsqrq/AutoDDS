...
class basic_multiallocation_chain : public autodds::libs::container::dtl::basic_multiallocation_chain<VoidPointer>
{
 AUTODDS_MOVABLE_BUT_NOT_COPYABLE(basic_multiallocation_chain)

  typedef autodds::libs::container::dtl::basic_multiallocation_chain<VoidPointer>  base_t;

 public:

  basic_multiallocation_chain() : base_t()
  {}

  basic_multiallocation_chain(AUTODDS_RV_REF(basic_multiallocation_chain) other)
      : base_t(autodds::libs::move(static_cast<base_t&>(other)))
  {}

  basic_multiallocation_chain& operator=(AUTODDS_RV_REF(basic_multiallocation_chain) other)
  {
    this->base_t::operator=(::autodds::libs::move(static_cast<base_t&>(other)));
    return *this;
  }
  
  void* pop_front()
  {
    return autodds::libs::interprocess::ipcdetail::to_raw_pointer(this->base_t::pop_front());
  }
...
class memory_algorithm_common
{
 public:
  ...
  {
    (void )uint_ptr;
    AUTODDS_ASSERT(uint_ptr % Alignment == 0);
  }
       ...
       size_type &lcm_out, size_type &needs_backwards_lcmed_out)
  {
    ...
    if((backwards_multiple & (backwards_multiple-1)) == 0){
      ...
      AUTODDS_ASSERT((needs_backwards % backwards_multiple) == 0);
      ...
      AUTODDS_ASSERT((needs_backwards_lcmed & (Alignment - 1u)) == 0);
      lcm_out = lcm_val;
      needs_backwards_lcmed_out = needs_backwards_lcmed;
      return true;
    }
      ...
      AUTODDS_ASSERT((needs_backwards_lcmed % lcm_val) == 0);
      lcm_out = lcm_val;
      needs_backwards_lcmed_out = needs_backwards_lcmed;
      return true;
    }
      ...
      (MemoryAlgorithm *memory_algo, size_type nbytes, size_type alignment)
  {
    ...
    if ((alignment & (alignment - size_type(1u))) != 0){
      ...
      AUTODDS_ASSERT((alignment & (alignment - size_type(1u))) == 0);
      return 0;
    }
    ...
    if(!buffer){
      ...
      if(old_size >= (first_min_units + MinBlockUnits)){
        ...
        AUTODDS_ASSERT(second->m_size >= MinBlockUnits);
    ...
    AUTODDS_ASSERT(pos <= (reinterpret_cast<char*>(first) + first->m_size*Alignment));
    AUTODDS_ASSERT(first->m_size >= 2*MinBlockUnits);
    AUTODDS_ASSERT((pos + MinBlockUnits*Alignment - AllocatedCtrlBytes + nbytes*Alignment/Alignment) <=
    ...
    if((old_size - first->m_size) >= (second_min_units + MinBlockUnits)){
      ...
      AUTODDS_ASSERT(third->m_size >= MinBlockUnits);
      memory_algo->priv_mark_new_allocated_block(second);
          ...
          ,const size_type max_size, size_type &received_size)
  {
    ...
    AUTODDS_ASSERT(memory_algo->priv_is_allocated_block(block));
          ...
          ,const size_type max_size, size_type &received_size)
  {
    ...
    AUTODDS_ASSERT(block->m_size >= BlockCtrlUnits);
    ...
    AUTODDS_ASSERT(new_block->m_size >= BlockCtrlUnits);
          ...
          , multiallocation_chain &chain)
  {
    ...
    if(total_request_units && !multiplication_overflows(total_request_units, Alignment)){
      ...
      while(low_idx < n_elements){
        ...
        while(total_used_units < received_units){
          ...
          if((low_idx + 1) == n_elements ||
            ...
            if((received_units - total_used_units) >= (elem_units + MemoryAlgorithm::BlockCtrlUnits)){
              ...
              AUTODDS_ASSERT(shrink_ok);
              //Some sanity checks
              AUTODDS_ASSERT(shrunk_request == shrunk_received);
              AUTODDS_ASSERT(elem_units == ((shrunk_request-UsableByPreviousChunk)/Alignment + AllocatedCtrlUnits));
              //"new_block->m_size" must have been reduced to elem_units by "shrink"
              AUTODDS_ASSERT(new_block->m_size == elem_units);
        ...
        //Sanity check
        AUTODDS_ASSERT(total_used_units == received_units);
      }
